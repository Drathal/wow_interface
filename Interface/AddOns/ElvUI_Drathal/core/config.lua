...
local ElvDrathal = E:NewModule("ElvUI_Drathal")
...
function DrathalInstall:ElvUI(theme)
	...
	E.global["general"]["mapAlphaWhenMoving"] = true
			...
			button1 = {
				...
				func = function()
					...
					if IsAddOnLoaded('BigWigs') then
						DrathalInstall:InstallBigWigs()
					end
					
					if IsAddOnLoaded('BugSack') then
						DrathalInstall:InstallAddonSimple("BugSackDB")
					end
					
					if IsAddOnLoaded('ErrorFilter') then
						DrathalInstall:InstallAddonAceProf("ErrorFilterDB")
					end
					
					if IsAddOnLoaded('Postal') then
						DrathalInstall:InstallAddonAceProf("Postal3DB")
					end
					
					if IsAddOnLoaded('Skada') then
						DrathalInstall:InstallAddonAceProf("SkadaDB")
					end

					if IsAddOnLoaded('AddOnSkins') then
						DrathalInstall:InstallAddonAceProf("AddOnSkinsDB")
					end
...
function DrathalInstall:ElvUI_Chat()
	...
	E.db["datatexts"]["panels"]["BottomMiniPanel"] = "Time (Drathal)"
