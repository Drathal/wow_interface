...
function Scan:ScanByName(name,itemid,options)
	...
	ZGV:Debug("&scan Scan:ScanByName|cffffee00 %s id %d",name,itemid)
	table.wipe(self.rawdata)
	...
	ZGV:Debug("&gold ScanByName for: "..queryname.." id="..(self.queried_by_id or "nil").." partial="..(self.queried_by_partial_name and "yes" or "no").." exact="..tostring(exact))
...
function Scan.EventHandler(frame,event,arg1,arg2)
	...
	elseif event=="AUCTION_ITEM_LIST_UPDATE" then
		...
		then
			...
			--[[
			if self:GetAuctionCount("list")<=PAGELENGTH then
	...
	elseif event=="ZYGOR_CSAQNAME_CHANGED" then
		ZGV:Debug("%s %s->%s",event,tostring(arg1),tostring(arg2))
		-- we can scan again! Good, let's assume this completes a query.
		if arg2 and self.state=="SS_QUERYING" then
			-- analyze the fucker
			if self.queried_by_name or self.queried_by_partial_name or self.queried_by_link then -- this is a slow scan, just go ahead and scan results.
				self:SetState("SS_SCANNING")
			else
...
function Scan:Work()
	...
	if csaq~=last_csaq then self.EventHandler(self.FWORK,"ZYGOR_CSAQNAME_CHANGED",last_csaq,csaq) end
	if csaq2~=last_csaq2 then self.EventHandler(self.FWORK,"ZYGOR_CSAQFAST_CHANGED",last_csaq2,csaq2) end
	last_csaq,last_csaq2=csaq,csaq2

	-- count AILUs, just for debugging
	if self.last_was_AILU then
		self.last_was_AILU=false
		self:Debug("AILU x"..self.consecutive_AILU_count)
	end
...
function Scan:ScanAuctions()  -- in state: SS_SCANNING
	...
	if batch>PAGELENGTH then  -- full scan? prepare to wipe.
		...
		self.scan_pages = math.ceil(total/PAGELENGTH)
	...
	for row=self.last_scanned_in_batch+1,batch do  repeat
		...
		if not (self.queried_by_id or self.queried_by_link or self.queried_by_partial_name) and (batch<total or total<=PAGELENGTH) and (not self.queried_by_name or not name:find(self.queried_by_name,1,true)) then
			...
			ZGV:Debug("&scan Scan:ScanAuctions, it's not our search, sorry! (row ".. row ..", queried for ".. self.queried_by_name ..", found "..(name or "unnamed").."... What the hell!?)")
			self:Crash()
...
function Scan:SetState(state)
	...
	if self.state=="SS_QUERYING" then
		...
		self.scan_pages=0
		self.last_scan_start_time = debugprofilestop()
	end
	...
	if state=="SS_IDLE" then
		...
		if not (self.queried_by_name or self.queried_by_partial_name or self.queried_by_link or self.WAITFORII_callbacks) then ZGV:Debug("&gold Scan: Sending abominable empty query")  QueryAuctionItems("chrzaszcz brzmi w trzcinie", nil, nil, 0, false, -1, false) end
