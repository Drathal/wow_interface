...
function Appraiser:MakeBuyTable()	
	...
	container.ShoppingAddButton = CHAIN(ui:Create("Button",container))
		...
		:SetFont(FONT,12)
		:SetText("Add item")
		:SetScript("OnClick", function(self) Appraiser:ToggleSearchFrame() end)
		:SetScript("OnEnter",function(self) 
			CHAIN(GameTooltip):SetOwner(self, "ANCHOR_TOP") 
...
function Appraiser:UpdateProgressBar()
	...
	--if self.ActiveTab == "Inventory" then
		if self.manualScanning then
			...
			if Scan.scan_pages>1 and Scan.scan_pages<1000 then scanprogress = (((Scan.scan_page-2)/Scan.scan_pages) + (1/Scan.scan_pages))*scanprogress end
		...
		elseif Scan.scan_pages>1 and Scan.scan_pages<1000 and (Scan.state=="SS_QUERYING" or Scan.state=="SS_SCANNING" or Scan.state=="SS_ANALYZING") then
			if Scan.scan_pages>1 and Scan.scan_pages<1000 then scanprogress = (((Scan.scan_page-2)/Scan.scan_pages) + (1/Scan.scan_pages)) end
