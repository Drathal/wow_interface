...
function Appraiser:Scan(mode)
	...
	elseif (mode=="scannew" or not mode) and ZGVG.Scan:CanScanByName() then 
		Appraiser:GetInventoryItems()
...
function Appraiser:ScanNextItem()
	if Appraiser.manualScanning then
		if ZGVG.Scan:CanScanByName() and ZGVG.Scan.state=="SS_IDLE" then
			...
			ZGV:Debug("&gold ScanNextItem manual: waiting to scan... "..tostring(ZGVG.Scan:CanScanByName()).." "..ZGVG.Scan.state)
		end
		self:Update()
	elseif Appraiser.manualBuyScanning then
		if ZGVG.Scan:CanScanByName() and ZGVG.Scan.state=="SS_IDLE" then
			...
			ZGV:Debug("&gold ScanNextItem manualbuy: waiting to scan... "..tostring(ZGVG.Scan:CanScanByName()).." "..ZGVG.Scan.state)
		end
		self:Update()
	else
		Appraiser.manualScanNextItem=false
...
local function UpdateHandler(self, elapsed)
	...
	if (delay~=last_delay or canscan~=last_canscan or delay2~=last_delay2 or canscan2~=last_canscan2 or ZGV.Gold.State~=last_state) then
