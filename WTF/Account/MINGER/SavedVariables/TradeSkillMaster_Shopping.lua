
TradeSkillMaster_ShoppingDB = {
	["g@ @sniperCustomPrice"] = "check(minprice - 100000g, 90% minprice, check(minprice - 50000g, 80% minprice, check(minprice - 10000g, 60% minprice, check(minprice - 5000g, 50% minprice, check(minprice - 1000g, 30% minprice, check(minprice - 500g, 20% minprice, check(minprice - 50g, 10% minprice)))))))",
	["g@ @maxDeSearchPercent"] = 1,
	["g@ @postUndercut"] = "1c",
	["g@ @postBidPercent"] = 0.95,
	["_scopeKeys"] = {
		["char"] = {
			"Darlaxle - Blackhand", -- [1]
			"Drathol - Blackhand", -- [2]
			"Dorthal - Blackhand", -- [3]
			"Drahdo - Blackhand", -- [4]
			"Draltar - Blackhand", -- [5]
			"Dronthal - Blackhand", -- [6]
			"Drathon - Blackhand", -- [7]
			"Modranthir - Blackhand", -- [8]
			"Drathal - Blackhand", -- [9]
			"Modrix - Blackhand", -- [10]
			"Draltair - Blackhand", -- [11]
			"Drahmoon - Madmortem", -- [12]
			"Drathia - Madmortem", -- [13]
			"Drathal - Madmortem", -- [14]
			"Dronthal - Madmortem", -- [15]
		},
		["realm"] = {
			"Blackhand", -- [1]
			"Madmortem", -- [2]
		},
		["profile"] = {
			"Default", -- [1]
		},
		["factionrealm"] = {
			"Horde - Blackhand", -- [1]
			"Alliance - Blackhand", -- [2]
			"Alliance - Madmortem", -- [3]
		},
	},
	["g@ @helpPlatesShown"] = {
		["auction"] = true,
	},
	["g@ @sniperVendorPrice"] = true,
	["_hash"] = 1861581,
	["g@ @savedSearches"] = {
		{
			["searchMode"] = "normal",
			["lastSearch"] = 1498311106,
			["name"] = "Binding of Intellect",
			["filter"] = "Binding of Intellect",
		}, -- [1]
		{
			["searchMode"] = "normal",
			["lastSearch"] = 1497646032,
			["name"] = "binding of mastery",
			["filter"] = "binding of mastery",
		}, -- [2]
		{
			["searchMode"] = "normal",
			["lastSearch"] = 1497479606,
			["name"] = "felslate",
			["filter"] = "felslate",
		}, -- [3]
		{
			["searchMode"] = "normal",
			["lastSearch"] = 1496531782,
			["name"] = "Saber's Eye of Intellect",
			["filter"] = "Saber's Eye of Intellect",
		}, -- [4]
		{
			["searchMode"] = "normal",
			["lastSearch"] = 1496531707,
			["name"] = "masterful shadowruby",
			["filter"] = "masterful shadowruby",
		}, -- [5]
		{
			["searchMode"] = "normal",
			["lastSearch"] = 1496531674,
			["name"] = "Masterful Shadowfury",
			["filter"] = "Masterful Shadowfury",
		}, -- [6]
		{
			["searchMode"] = "normal",
			["lastSearch"] = 1496530495,
			["name"] = "Trigger/exact",
			["filter"] = "Trigger/exact",
		}, -- [7]
		{
			["searchMode"] = "normal",
			["lastSearch"] = 1496430336,
			["name"] = "Lovely Black Dress/exact",
			["filter"] = "Lovely Black Dress/exact",
		}, -- [8]
	},
	["g@ @sniperSound"] = "TSM_CASH_REGISTER",
	["g@ @marketValueSource"] = "dbmarket",
	["g@ @maxDeSearchLvl"] = 735,
	["g@ @minDeSearchLvl"] = 1,
	["g@ @normalPostPrice"] = "150% dbmarket",
	["_version"] = 3,
	["_currentProfile"] = {
		["Drathal - Blackhand"] = "Default",
		["Dronthal - Blackhand"] = "Default",
		["Dorthal - Blackhand"] = "Default",
		["Drathon - Blackhand"] = "Default",
		["Drathol - Blackhand"] = "Default",
		["Draltar - Blackhand"] = "Default",
		["Drathia - Madmortem"] = "Default",
		["Modranthir - Blackhand"] = "Default",
		["Dronthal - Madmortem"] = "Default",
		["Drahdo - Blackhand"] = "Default",
		["Modrix - Blackhand"] = "Default",
		["Drathal - Madmortem"] = "Default",
		["Drahmoon - Madmortem"] = "Default",
		["Draltair - Blackhand"] = "Default",
		["Darlaxle - Blackhand"] = "Default",
	},
}
